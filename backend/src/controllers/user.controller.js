...
export async function getRecommendeddUsers(req, res) {
    try {
        ...
        const curremtIser = req.user;

        const recommendedUsers = await User.find({
            $and: [
                { _id: { $ne: currentUserId } },
                { _id: { $nin: curremtIser.friends } },
                { isOnboarded: true }
            ]
        })
        res.status(200).json(recommendedUsers);
    } catch (error) {
        console.error("Error in getRecommendeddUsers controller", error.message);
        res.status(500).json({ message: "Internal Server Error" });
    }
}


export async function getMyFiends(req, res) {
    try {
        ...
        console.error("Error in getMyFiends controller", error.message);
...
export async function sendFriendRequest(req, res) {
    try {
        ...
        // prevent sending req to youself
        if (myId === recipientId) {
